...
Дополнительные ситуации, обработанные после релиза:
* **Cookie-баннер перекрывает сайт (REV-33):** закрывается до скриншотов через API платформы согласия, текст кнопки или скрытие оверлея; бюджет 3 с, при неудаче аудит продолжается, итог пишется в `cookieBannerHandled`.
* **Длинная страница с lazy-контентом (REV-21):** автопрокрутка, принудительный показ анимаций, ограничение высоты полностраничного скриншота.
* **LLM вернул слишком длинное поле (REV-34):** строка обрезается по границе слова до лимита схемы вместо отказа от всего ответа.
* **Генерация MVP упала после всех ретраев (REV-31):** лид возвращается в `AUDITED` или `NEEDS_APPROVAL`, ошибка показывается оператору, генерацию можно повторить.
* **Судья полноты недоступен или ошибся (REV-37):** откат на сравнение кодом, причина в `llmError`; сбой самого сравнения — отчет `unverified`.
* **Провайдер карт недоступен или неверно настроен (REV-26):** таймауты Overpass повторяются, ошибки конфигурации завершают задачу без повторов с понятным сообщением.
...
### ADR 5: Контент MVP только с исходного сайта (REV-23)
* **Статус:** Принято.
* **Контекст:** Без LLM-ключа все MVP получали одинаковые шаблонные тексты, выдуманные отзывы и метрики, а также плейсхолдерные контакты (телефон, город, часы работы). Такие MVP нельзя показывать владельцу бизнеса.
* **Решение:** Детерминированный экстрактор внутри страницы (`site-content.extractor.ts`) копирует контент исходного сайта и schema.org JSON-LD; он становится единственным источником фактов для LLM и для детерминированного fallback. Шаблон рендерит блоки с фактами только при наличии данных.
* **Последствия:** Каждый MVP уникален и проверяем; страницы с малым количеством контента выглядят беднее, но не содержат неправды.

### ADR 6: Генерация MVP на языке исходного сайта (REV-25)
* **Статус:** Принято (заменяет перевод продукта на английский из REV-22 в части MVP).
* **Контекст:** Владельцу польской или литовской клиники нужен прототип на языке его сайта, а не на английском.
* **Решение:** LLM получает `outputLanguage` (тег `lang` сайта или язык его текста) и пишет на нем; шаблон выставляет BCP 47-тег сайта в `<html lang>` и локализует фиксированный UI-текст для en, ru, be, pl, lt. Интерфейс оператора — отдельно, на выбранном оператором языке (REV-24).
* **Последствия:** Для языков без словаря UI шаблона остается английским при корректном `lang`; для расширения достаточно добавить словарь в `mvp-locale.ts`.

### ADR 7: Поиск бизнесов через официальные API карт и ревью оператором (REV-26, REV-29)
* **Статус:** Принято.
* **Контекст:** Ручной ввод URL не масштабируется. Парсинг страниц Google Maps нарушает ToS Google; автоматический импорт всех найденных бизнесов засоряет воронку.
* **Решение:** OpenStreetMap (Nominatim + Overpass, бесплатно, ODbL) по умолчанию и Google Places API (New) по ключу. Воркер только классифицирует листинги; лиды создаются из кандидатов, выбранных оператором, причем данные берутся из результата задачи, а не из запроса.
* **Последствия:** Соблюдение правил провайдеров (User-Agent, конкурентность 1), контроль оператора над воронкой. Результаты поиска живут в Redis вместе с задачей BullMQ и не хранятся в MongoDB.

### ADR 8: Выбор LLM-провайдера на каждую генерацию и локальный Claude Code CLI (REV-30, REV-32)
* **Статус:** Принято.
* **Контекст:** Разработка и демонстрации требуют генерации без оплаты API, а качество текстов зависит от модели. Ключи API и бинарник CLI находятся на хосте воркеров, а не API.
* **Решение:** Общий каталог провайдеров и моделей в `@revamp/shared-types`; оператор выбирает провайдера и модель в диалоге генерации. Воркеры публикуют доступность провайдеров в Redis, API отдает ее дашборду. Провайдер `claude-cli` запускает локальный `claude -p` под залогиненным аккаунтом в изолированном режиме. При сбое — только детерминированный fallback, без переключения на другой платный провайдер.
* **Последствия:** Предсказуемые расходы; `MvpProject` фиксирует выбранную и фактическую модель, поэтому качество можно сравнивать. CLI-провайдер зависит от сессии на хосте воркеров и не подходит для масштабируемого продакшена.

### ADR 9: Проверка полноты MVP: LLM судит, код проверяет (REV-36, REV-37)
* **Статус:** Принято.
* **Контекст:** Оператору нужно быстро понять, не потерял ли MVP телефон, адрес или услуги и не выдумал ли контакты. Чистое сравнение кодом ошибается на перефразированных часах работы и переведенных названиях услуг; чистое суждение LLM непроверяемо.
* **Решение:** Код разбирает HTML MVP и сравнивает его с данными аудита. LLM (если включен) судит поля, но каждый вердикт обязан цитировать MVP дословно; код принимает вердикт только после проверки цитаты, а балл и `not_in_source` считает сам. Любой сбой LLM — откат на сравнение кодом.
* **Последствия:** Отчет устойчив к перефразированию и при этом не может содержать неподтвержденных утверждений. Отчет рекомендательный и не блокирует HITL-одобрение, но при критических проблемах требует дополнительного подтверждения оператора.
...
* Дорожная карта и развитие после релиза: [`milestones.md`](./milestones.md)
* Системные промпты ИИ-агентов: [`AGENTS.md`](./AGENTS.md)

...
> **Снимок на момент REV-20 (24.09.2026).** Отчет фиксирует состояние после первоначальной 4-недельной дорожной карты и не обновляется. Изменения после релиза (`REV-21` — `REV-41`: поиск бизнесов, контент и язык исходного сайта, проверка полноты MVP, выбор LLM-провайдера и др.) описаны в [`milestones.md`](./milestones.md#6-развитие-после-релиза-rev-21--rev-41), актуальная архитектура — в [`blueprint.md`](./blueprint.md).
